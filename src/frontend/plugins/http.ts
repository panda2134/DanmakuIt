...
type APIGetType<Q extends paths[keyof paths]> = Q extends Record<'get', any> ? {
  get: (...args: string[]) => Promise<Q['get']['responses'] extends {200: any} ?
...
type APIDeleteType<Q extends paths[keyof paths]> = Q extends Record<'delete', any> ? {
  delete: (...args: string[]) => Promise<Q['delete']['responses'] extends {200: any} ?
...
type APIPatchType<Q extends paths[keyof paths]> = Q extends Record<'patch', any> ? {
  patch: (postBody: Q['patch']['requestBody']['content']['application/json'], ...args: string[]) =>
...
type APIPostType<Q extends paths[keyof paths]> = Q extends Record<'post', any> ? {
  post: (postBody: Q['post']['requestBody']['content']['application/json'], ...args: string[]) =>
...
function getAPI ({ $axios }: Context): APIType {
  return {
    ...
    '/room/{room_id}/client-login': {
      get: (roomId, roomPasscode) => $axios.$get(`/room/${roomId}/client-login`, {
        headers: { Authorization: `Bearer ${roomPasscode}` }
      })
    },
    '/room/{room_id}': {
      get: roomId => $axios.$get(`/room/${roomId}`),
      delete: roomId => $axios.$delete(`/room/${roomId}`),
      patch: (postBody, roomId) => $axios.$patch(`/room/${roomId}`, postBody)
    },
    '/room/': {
      get: () => $axios.$get('/room/'),
